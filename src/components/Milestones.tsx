...
export default function Milestones() {
  ...
  return (
    <section 
      ...
      className="py-20 md:py-24 px-4 sm:px-6 md:px-10 lg:px-12 xl:px-16 2xl:px-20 bg-slate-50 text-slate-900 border-b border-slate-200 transition-colors duration-500"
    >
      <div className="w-full max-w-[1600px] 2xl:max-w-[1720px] mx-auto space-y-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          ...
          {/* Switcher Tabs */}
          <div className="flex items-center gap-1.5 p-1 rounded-2xl glass-card border border-slate-200 self-start lg:self-end pb-2 select-none">
            <button
        ...
        <div className="p-6 sm:p-8 rounded-3xl glass-card border border-slate-200 bg-white shadow-xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between pb-6 border-b border-slate-200 text-xs font-mono">
            ...
            <span className="text-slate-500 font-medium">100% Zero Defect Release Audit</span>
          </div>

          <div className="h-80 sm:h-96 md:h-[420px] w-full pt-6 min-w-0 min-h-[320px]">
            {isMounted ? (
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={300}>
                {activeTab === "growth" ? (
                  <AreaChart data={growthTimelineData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="year" stroke="#64748b" fontSize={11} fontFamily="monospace" />
                  ...
                  <BarChart data={efficiencyImpactData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#cbd5e1" opacity={0.6} />
                    <XAxis dataKey="metric" stroke="#64748b" fontSize={10.5} fontFamily="monospace" />
                    <YAxis stroke="#64748b" fontSize={11} fontFamily="monospace" />
                    <Tooltip content={<CustomImpactTooltip />} />
                    <Bar dataKey="beforeSkill" name="Legacy Baseline" fill="#94a3b8" radius={[6, 6, 0, 0]} />
          ...
          {/* Legend row */}
          <div className="flex flex-wrap items-center justify-center gap-6 pt-6 border-t border-slate-200 text-xs font-mono">
            {activeTab === "growth" ? (
              <>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-sky-500 inline-block" />
                  <span className="text-slate-700">Carrier Subscribers (Millions)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-emerald-500 inline-block" />
                  <span className="text-slate-700">Automated Test Suites</span>
                </div>
              </>
            ) : (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700">Legacy Architecture Baseline</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-sm bg-sky-600 inline-block" />
                  <span className="text-slate-700">Optimized Solution Stream</span>
                </div>
              </>
            )}
          </div>
